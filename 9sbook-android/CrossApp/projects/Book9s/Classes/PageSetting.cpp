...
void PageSetting::viewDidLoad()
{
	...
	pBk2->setFrame( CCRect( 0 , 56 , 0 , 0 ) );
	getView()->addSubview( pBk2 );

	Setting setting = BookDB::Instance()->GetSetting();

	m_pFontSize = CASegmentedControl::createWithFrame( CCRect( 167 , 41 , 511 , 52 ) , 4 );
	for ( int i = 0 ; i < 4 ; i ++ )
	{
	...
	m_pFontColor = CASegmentedControl::createWithFrame( CCRect( 170 , 177 , 491 , 97 ) , 4 );
	for ( int i = 0 ; i < 4 ; i ++ )
	{
	...
	m_pBkColor = CASegmentedControl::createWithFrame( CCRect( 170 , 369 , 491 , 97 ) , 4 );
	for ( int i = 0 ; i < 4 ; i ++ )
	{
