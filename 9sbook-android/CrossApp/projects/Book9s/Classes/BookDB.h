...
typedef void (CCObject::*SEL_BookDBOnComplete)( void *pData );
#define BookDB_selector(_SELECTOR) (SEL_BookDBOnComplete)(&_SELECTOR)
class BookDBHttpRequest;
typedef void (CCObject::*SEL_BookDBOnHttpComplete)( BookDBHttpRequest *pRequest , CSJson::Value *pJson );
...
{
public:
	CCObject *pTarget; // 回调目标
	SEL_BookDBOnComplete onComplete; // 回调函数
	SEL_BookDBOnHttpComplete onHttpComplete; // HTTP完成函数
	void *pUserData; // 用户数据

	BookDBHttpRequest( CCObject *pTarget , SEL_BookDBOnComplete onComplete , SEL_BookDBOnHttpComplete onHttpComplete , void *pUserData )
	{
...
class BookDB : public CCObject
{
	...
	void HttpRequest( std::string url , CCObject *pTarget , SEL_BookDBOnComplete onComplete , SEL_BookDBOnHttpComplete onHttpComplete , void *pUserData = NULL , int nThread = 0 );
...
public:
	...
	// 打开排行
	void OpenRank( CCObject *pTarget , SEL_BookDBOnComplete onComplete );
