...
void PageBookStorage::OnBookIconHold( BookIcon *pBook )
{
	...
	CAButton *pButton = CAButton::createWithFrame( CCRect( 0 , PageManager::Instance()->GetMessageBoxLayer()->getFrame().size.height - 116 , 359 , 116  ) , CAButtonTypeCustom );
	...
	pButton->addTarget( this , CAControl_selector(PageBookStorage::OnButton3Down) , CAControlTouchUpInSide );
	PageManager::Instance()->GetMessageBoxLayer()->addSubview( pButton );

	pButton = CAButton::createWithFrame( CCRect( 359 ,PageManager::Instance()->GetMessageBoxLayer()->getFrame().size.height - 116 , 361 , 116  ) , CAButtonTypeCustom );
...
void PageBookStorage::Refresh()
{
	...
	if ( vec.size() == 0 )
	{
		...
		pBk2->setFrame( CCRect( 0 , 0 , 720 , 1114 ) );
		getView()->addSubview( pBk2 );

		CAButton *pButton = CAButton::createWithFrame( CCRect( 0 , 500 , 720 , 188  ) , CAButtonTypeCustom );
		pButton->setBackGroundViewForState( CAControlStateNormal , CAScale9ImageView::createWithImage( CAImage::create( "StorageButton1.png" ) ) );
		pButton->setBackGroundViewForState( CAControlStateHighlighted , CAScale9ImageView::createWithImage( CAImage::create( "StorageButton1light.png" ) ) );
		pButton->addTarget( this , CAControl_selector(PageBookStorage::OnButton1Down) , CAControlTouchUpInSide );
		getView()->addSubview( pButton );

		pButton = CAButton::createWithFrame( CCRect( 0 , 690 , 720 , 188  ) , CAButtonTypeCustom );
		pButton->setBackGroundViewForState( CAControlStateNormal , CAScale9ImageView::createWithImage( CAImage::create( "StorageButton2.png" ) ) );
		pButton->setBackGroundViewForState( CAControlStateHighlighted , CAScale9ImageView::createWithImage( CAImage::create( "StorageButton2light.png" ) ) );
		...
		if ( vec.size() > 0 )
		{
			...
			for ( int j = 0 ; ; j ++ )
			{
				for ( int i = 0 ; i < 3 ; i ++ )
				{
					...
					pIcon->init();
					pIcon->setFrame( CCRect( i * 230 + 36 , j * 308 + 36 , 0 , 0 ) );
					...
					int y = pIcon->getFrame().origin.y + pIcon->getFrame().size.height + 50;
