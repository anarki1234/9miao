...
void PageReader::viewDidLoad()
{
	...
	m_pMenuName = CALabel::createWithFrame( CCRect( 0 , 0 , 720 , 80 ) );
	...
	m_pMenuName->setFontSize( setting.m_nTextSize );
...
void PageReader::OnComplete( void *pData )
{
	...
	if ( pData != NULL && m_pScrollView != NULL )
	{
		...
		{
			m_pButton = CAButton::createWithFrame( CCRect( (720 - 250)/2 , ypos , 250 , 80 ) , CAButtonTypeCustom );
